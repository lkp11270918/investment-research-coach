...
from backend.models import CompanyProfile, EvidenceGraph, EvidenceGraphNode, ResearchProjectCreate, ThesisDraft, ThesisScenario, ThesisVariable, ThesisVersion, VerificationStatus
...
class ThesisBuilderTest(unittest.TestCase):
    ...
    def test_complete_thesis_passes_and_versions_persist(self) -> None:
        graph = EvidenceGraph(nodes=[
            EvidenceGraphNode(node_id="EVIDENCE:E1", node_type="financial_fact", label="现金流稳定", evidence_id="E1", verification_status=VerificationStatus.VERIFIED),
            EvidenceGraphNode(node_id="EVIDENCE:E2", node_type="risk", label="资本开支上升", evidence_id="E2", verification_status=VerificationStatus.VERIFIED),
            EvidenceGraphNode(node_id="EVIDENCE:E3", node_type="fact", label="需求保持稳定", evidence_id="E3", verification_status=VerificationStatus.VERIFIED),
            EvidenceGraphNode(node_id="EVIDENCE:E4", node_type="financial_fact", label="自由现金流覆盖分红", evidence_id="E4", verification_status=VerificationStatus.VERIFIED),
        ])
    ...
    def test_sell_side_only_and_public_rating_fail(self) -> None:
        graph = EvidenceGraph(nodes=[EvidenceGraphNode(node_id="EVIDENCE:S1", node_type="sell_side_opinion", label="卖方看好", evidence_id="S1", verification_status=VerificationStatus.VERIFIED)])
    ...
    def test_irrelevant_evidence_cannot_create_false_coverage(self) -> None:
        graph = EvidenceGraph(nodes=[EvidenceGraphNode(node_id="EVIDENCE:X", node_type="fact", label="办公地址位于上海", evidence_id="X", verification_status=VerificationStatus.VERIFIED)])
