...
class ResearchDepthTest(unittest.TestCase):
    def test_structured_valuation_has_three_scenarios_and_margin(self):
        from backend.models import ValuationAssumptions
        result=analyze_valuation([ev("share_price",10),ev("eps",1),ev("free_cash_flow_to_firm",100),ev("shares_outstanding",10),ev("interest_bearing_debt",20),ev("cash_and_equivalents",5)],assumptions=ValuationAssumptions(confirmed=True))
